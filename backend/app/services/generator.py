...
def _build_client() -> AzureOpenAI:
    ...
    if not endpoint and not base_url:
        raise ValueError('未配置 Azure AI 地址，请配置 AZURE_ENDPOINT_URL 或 AZURE_BASE_URL')
...
def _chat_completion_with_model_fallback(client: AzureOpenAI, **kwargs):
    fallback_models = [x.strip() for x in (settings.azure_deployment_fallbacks or '').split(',') if x.strip()]
    ...
    for model_name in model_candidates:
        try:
            ...
            if ('Resource not found' in text) or ("'code': '404'" in text) or ('Error code: 404' in text):
                continue
            raise

    raise ValueError(f'AI 请求失败：可用部署均不可用（{",".join(model_candidates)}），最后错误: {last_error}')
    ...
    if isinstance(completion, dict):
        ...
        if choices:
            ...
            if isinstance(content, list):
                parts = [str(x.get('text') or '') for x in content if isinstance(x, dict)]
...
def _resolve_speech_tts_endpoint() -> str:
    ...
    tts_path = (settings.azure_speech_tts_path or '/tts/cognitiveservices/v1').strip() or '/tts/cognitiveservices/v1'
    if endpoint:
...
def _synthesize_with_azure_speech_service(script_text: str, language_key: str, language_label: str) -> str:
    ...
    if not key or not endpoint:
        raise ValueError('未配置 Azure Speech 参数，请先配置 AZURE_SPEECH_KEY 与 AZURE_SPEECH_ENDPOINT/AZURE_SPEECH_REGION')
    ...
    def _call_once(sub_key: str) -> bytes:
        req = urllib.request.Request(endpoint, data=body, headers={**headers, 'Ocp-Apim-Subscription-Key': sub_key}, method='POST')
        if settings.azure_ssl_skip_verify:
            context = ssl._create_unverified_context()
        else:
            context = ssl.create_default_context()
        with urllib.request.urlopen(req, timeout=settings.azure_request_timeout_sec, context=context) as resp:
            return resp.read()
    ...
    try:
        ...
        if key2 and exc.code in (401, 403, 429):
            try:
                ...
                raise ValueError(f'Azure Speech 合成失败({language_key}/{language_label}): {exc2}') from exc2
        else:
            detail = exc.read().decode('utf-8', errors='ignore')
            raise ValueError(f'Azure Speech 合成失败({language_key}/{language_label}): HTTP {exc.code} {detail}') from exc
    except Exception as exc:
        raise ValueError(f'Azure Speech 合成失败({language_key}/{language_label}): {exc}') from exc
...
def generate_finance_reflections(title_text: str, summary_text: str, script_text: str) -> list[str]:
...
def generate_script_and_highlights(summary_raw: str, speaker: str, title: str = '') -> tuple[str, list[str], list[str]]:
    if not summary_raw.strip():
        raise ValueError('内容不能为空')
    client = _build_client()
    prompt = (
        '你是新闻口播编辑，同时具备国际金融与财务分析背景。'
        '请将会议总结改写为通俗口播稿，并提取2条亮点，同时给出3到5条金融专家反思建议。'
        '反思建议要覆盖公司治理、财务、全球金融等角度，内容具体可执行。'
        '必须严格返回 JSON，不要输出任何额外文字。'
        '格式为：{"script":"...","highlights":["...","..."],"reflections":["...","...","..."]}'
    )
    try:
        completion = _chat_completion_with_model_fallback(
            client,
    ...
    if len(reflections) < 3:
        reflections = generate_finance_reflections(title_text=title, summary_text=summary_raw, script_text=script)

    return script, highlights, reflections
...
def translate_report_package(title_text: str, script_text: str, highlights: list[str], target_language: str) -> tuple[str, str, list[str]]:
    title = title_text.strip()
    script = script_text.strip()
    ...
    try:
        completion = _chat_completion_with_model_fallback(
            ...
            messages=[
                {'role': 'system', 'content': [{'type': 'text', 'text': prompt}]},
                {'role': 'user', 'content': [{'type': 'text', 'text': json.dumps(source_payload, ensure_ascii=False)}]},
            ],
    ...
    if not isinstance(highlights_raw, list):
        highlights_raw = []
    highlights_out = [str(x).replace('```', '').strip() for x in highlights_raw if str(x).strip()][:2]
...
def generate_reflection_qa(title_text: str, summary_text: str, script_text: str) -> list[tuple[str, str]]:
    title = title_text.strip()
    summary = summary_text.strip()
    script = script_text.strip()
    source_text = script or summary
    ...
    fallback = [
        ('这次会议最核心的目标是否清晰且可量化？', f'建议围绕“{key_point}”补充可量化指标与截止时间。'),
        ('执行过程中最大的风险点是什么？', '建议按优先级列出前三个风险，并明确责任人与缓解动作。'),
        ('下一次会议前，最关键的一项行动是什么？', '建议确定一个可在本周完成的行动项，并在下次会议做结果复盘。'),
    ]
    return fallback


def generate_meeting_chapters_from_transcript(title_text: str, transcript_text: str) -> list[str]:
    title = title_text.strip()
...
def synthesize_script_audio_pcm_base64(script_text: str, language_key: str, language_label: str) -> str:
    text = script_text.strip()
    if not text:
        raise ValueError('口播稿不能为空')
    ...
    if (settings.azure_speech_key or '').strip():
        return _synthesize_with_azure_speech_service(
            script_text=text,
    ...
    if not deployment:
        raise ValueError('未配置 TTS：请设置 AZURE_SPEECH_KEY 或 AZURE_TTS_DEPLOYMENT_NAME')
    client = _build_client()
    ...
    try:
        ...
        raise ValueError(f'AI 语音合成失败({language_key}/{language_label}): {exc}') from exc
