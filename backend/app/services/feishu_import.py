...
class FeishuApiClient:
    ...
    def _request_json(
        ...
        status, _, raw = self._http_request(
    ...
    def get_tenant_access_token(self) -> str:
        ...
        if payload.get("code") != 0:
            raise RuntimeError(
                f"获取飞书 tenant_access_token 失败: {payload.get('msg')}"
            )
    ...
    @staticmethod
    def parse_docx_token(docx_url: str) -> str:
        """解析飞书文档链接，提取document_id（文档token）

        支持格式：
        - https://tpc.feishu.cn/docx/SH0TdNcUkot3Jfx5VJOcPfNknIv
        - https://example.feishu.cn/docx/xxxxx
        """
        raw = (docx_url or "").strip()
        # 匹配 /docx/[token] 格式
        match = re.search(r"/docx/([A-Za-z0-9_-]{8,})", raw)
        return match.group(1) if match else ""
    ...
    def _walk_text_nodes(
        node: Any, path: list[str], out: list[tuple[list[str], str]]
    ) -> None:
    ...
    def list_meetings_by_no(
        self, meeting_no: str, start_ts: int, end_ts: int
    ) -> list[dict[str, Any]]:
        page_token = ""
        ...
        for _ in range(max_pages):
            ...
            payload = self._request_json(
                "GET", "/open-apis/vc/v1/meetings/list_by_no", query=query
            )
            if payload.get("code") != 0:
    ...
    def get_minute(self, minute_token: str) -> dict[str, Any]:
        payload = self._request_json(
            "GET", f"/open-apis/minutes/v1/minutes/{minute_token}"
        )
        if payload.get("code") != 0:
            return {
                "code": payload.get("code"),
    ...
    def get_docx_raw_content(self, document_id: str) -> dict[str, Any]:
        ...
        Args:
            document_id: 文档token（从文档链接中解析得到）

        Returns:
            dict包含：
            - code: 错误码（0表示成功）
            - msg: 错误信息
            - content: 文档原始内容（JSON格式）
        """
        payload = self._request_json(
        ...
        data = payload.get("data") or {}
        return {"code": 0, "msg": "", "content": data.get("content") or {}}

    @classmethod
    def extract_docx_text(cls, raw_content: Any) -> str:
        """从飞书文档原始内容中提取纯文本

        Args:
            raw_content: get_docx_raw_content返回的content字段
        ...
        # 收集所有文本节点
        text_nodes: list[tuple[list[str], str]] = []
        cls._walk_text_nodes(raw_content, [], text_nodes)

        # 提取文本内容
        lines = []
        for path, text in text_nodes:
            # 过滤掉一些元数据字段
            if any(x in path for x in ["block_id", "parent_id", "children"]):
                continue
            lines.append(text)

        # 去重和清理
        cleaned_lines = cls._dedupe_lines(lines, limit=10000)
        return "\n".join(cleaned_lines)

    def fetch_items_from_meeting_url(
        self, meeting_url: str, lookback_days: int = 30
    ) -> list[FeishuMeetingImportItem]:
        meeting_no = self.parse_meeting_no(meeting_url)
        end_time = int(datetime.now().timestamp())
        start_time = int(
            (datetime.now() - timedelta(days=max(1, int(lookback_days)))).timestamp()
        )
        briefs = self.list_meetings_by_no(
        ...
        for brief in briefs:
            ...
            if minute_token:
                ...
                minute_summary_formal, minute_summary_draft = (
                    self.extract_minute_summaries(minute)
                )
                transcript_status, transcript_text, transcript_error = (
                    self.get_transcript(minute_token=minute_token)
                )
            else:
                recording_msg = str(recording_payload.get("msg") or "").strip()
                meeting_status = str(detail.get("status") or "").strip()
                if (
                    recording_msg.lower() == "meeting status unexpected"
                    or meeting_status == "2"
                ):
    ...
    def fetch_items_from_minutes_url(
        self, minutes_url: str
    ) -> list[FeishuMeetingImportItem]:
        ...
        minute_title = str(minute.get("title") or "").strip()
        minute_owner_id = str(minute.get("owner_id") or "").strip()
        minute_summary_formal, minute_summary_draft = self.extract_minute_summaries(
            minute
        )
        ...
        transcript_status, transcript_text, transcript_error = self.get_transcript(
            minute_token=minute_token
        )
        minute_error = str(minute_payload.get("msg") or "").strip()
        if (
            minute_payload.get("code") != 0
            and minute_error
            and minute_error not in transcript_error
        ):
    ...
    def fetch_items_from_url(
        self, source_url: str, lookback_days: int = 30
    ) -> list[FeishuMeetingImportItem]:
        minute_token = self.parse_minutes_token(source_url)
        if minute_token:
            return self.fetch_items_from_minutes_url(source_url)
        return self.fetch_items_from_meeting_url(
            meeting_url=source_url, lookback_days=lookback_days
        )
