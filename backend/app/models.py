...
class MeetingReport(Base):
    ...
    source_language: Mapped[str] = mapped_column(String(16), default='zh', nullable=False)
    ...
    auto_play_enabled: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    status: Mapped[str] = mapped_column(String(20), default='draft', nullable=False)
    source_type: Mapped[str] = mapped_column(String(32), default='', nullable=False)
    source_meeting_no: Mapped[str] = mapped_column(String(64), default='', nullable=False)
    source_meeting_id: Mapped[str] = mapped_column(String(64), default='', nullable=False)
    source_minute_token: Mapped[str] = mapped_column(String(128), default='', nullable=False)
...
class MeetingReportHighlight(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    report_id: Mapped[int] = mapped_column(Integer, ForeignKey('meeting_reports.id', ondelete='CASCADE'), index=True)
...
class MeetingReportTranslation(Base):
    ...
    __table_args__ = (UniqueConstraint('report_id', 'language_key', name='uq_report_language'),)
    ...
    audio_pcm_base64: Mapped[str] = mapped_column(Text().with_variant(LONGTEXT, 'mysql'), default='', nullable=False)
...
class PlaybackRuntimeSetting(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    mode: Mapped[str] = mapped_column(String(32), default='carousel_summary', nullable=False)
    carousel_scope: Mapped[str] = mapped_column(String(16), default='loop', nullable=False)
